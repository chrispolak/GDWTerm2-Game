...
public class TileMaster : EditorWindow {
	...
	static void ResetManager()
	{
		...
		if(cmquad == null)
		{
			...
			cmquad.renderer.enabled = false; //disable quad's renderer
			
			EditorUtility.SetSelectedWireframeHidden(cmquad.renderer, true); //hide wireframe from wireframe view mode
		}
		if(window != null)
		{
